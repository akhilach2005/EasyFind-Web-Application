  ...
  if (!empty($_FILES['file']['name'])) {
    ...
    if(!move_uploaded_file($tempname,$folder))
    {
      ...
      exit();
        ...
        $query="INSERT INTO FoundItems(RegistrationId,ItemName,Category,Location_Found,Date_Found,Contact,Description,file)
        VALUES('$username','$ItemName','$category','$location','$date','$contact','$description','$file_name')";
        if(!mysqli_query($conn,$query))
        {
            echo "Could not insert. ".mysqli_connect_error();
        }
        else {
            echo "<script>document.addEventListener('DOMContentLoaded', function() {
                document.getElementById('sm').innerText='Form Submitted successfully!';
            });</script>";
            
            $row="SELECT Student_Name FROM Students WHERE RegistrationID='$username'";
            ...
            if($res && mysqli_num_rows($res)>0){
              ...
              $check="SELECT * FROM Rewards WHERE RegistrationID='$username'";
              $checkResult=mysqli_query($conn,$check);
                    if($checkResult && mysqli_num_rows($checkResult)>0)
                    {
                      $update="UPDATE Rewards 
                              SET Points=Points+10
                              WHERE RegistrationID='$username'";
                      $updateResult=mysqli_query($conn,$update) ;
                        if($updateResult && mysqli_affected_rows($conn)==1)
                        {
                          echo "Updation Successful!!";
                          $p="SELECT* FROM Rewards WHERE RegistrationID='$username'";
                          $pResult=mysqli_query($conn,$p);
                                    if($pResult && mysqli_num_rows($pResult)>0)
                                    {
                                      $PointsRow=mysqli_fetch_assoc($pResult);
                                      $points=$PointsRow['Points'];
                                                if($points<=20)
                                                {
                                                  $badge="UPDATE Rewards 
                                                  SET Badges='Helper'
                                                    WHERE RegistrationID='$username'";
                                                }
                                                ...
                                                $badge="UPDATE Rewards 
                                                SET Badges='Campus Hero'
                                                  WHERE RegistrationID='$username'";
                                                }
                                                else{
                                                  $badge="UPDATE Rewards 
                                                  SET Badges='No badges'
                                                    WHERE RegistrationID='$username'";
                                                }
                                               if(! mysqli_query($conn,$badge)){
                                                echo "Badge update failed!".mysqli_error($conn);
                                               }
                                              }
                          }
                          else{
                ...
                $ins="INSERT INTO Rewards (RegistrationID,Name,Points,Badges)
                VALUES 
                ('$username','$studentName',10,'No Badges')";
...
<body>
    ...
    <div class="form-wrapper">
        ...
        <div id="lostItemForm">
            <h2 class="form-title" style="align-items: center;">Report the found Item</h2>
            <form action="reportform.php" method="POST" enctype="multipart/form-data" onsubmit="validateForm()">
                ...
                <label for="location">Location (where you found it):</label>
                <input type="text" id="location" placeholder="Enter the location" name="location" required>
                <label for="date">Date(when you found it):</label>
...
function validateForm()
{
  ...
  if (isValid) {
    form.submit(); // safely submit the form now
  }
}
